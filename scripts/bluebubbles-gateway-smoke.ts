...
interface Report {
  bluebubblesRequests: BlueBubblesRequestRecord[];
  failedChecks: string[];
...
interface BlueBubblesRequestRecord {
  body: string;
...
const tempRoot = mkdtempSync(join(tmpdir(), "flyflor-bluebubbles-smoke-"));
const cliHome = resolve(tempRoot, "cli-home");
const gatewayLogPath = resolve(cliHome, "logs", "gateway.log");
const password = "bluebubbles-password";
const chatGuid = "iMessage;-;+15550001";
const userAddress = "+15550001";
const outputDir = resolve(root, ".flyflor-cli", "bluebubbles-smoke", timestamp());

mkdirSync(outputDir, { recursive: true });

const bluebubblesRequests: BlueBubblesRequestRecord[] = [];
const kernelMessages: unknown[] = [];
let gateway: ReturnType<typeof spawn> | undefined;
let cleanup: (() => void) | undefined;

try {
  const bluebubbles = await startBlueBubblesServer(bluebubblesRequests);
  const kernel = await startKernelServer(kernelMessages);
  cleanup = () => {
    kernel.close();
    bluebubbles.server.close();
  };
  gateway = spawn("cargo", ["run", "--quiet", "--", "gateway", "run"], {
    cwd: root,
    env: {
      ...process.env,
      BLUEBUBBLES_ALLOWED_USERS: userAddress,
      BLUEBUBBLES_INBOUND_WEBHOOK: JSON.stringify({
        messages: [
          {
            chat: { guid: chatGuid, isGroup: false },
            guid: "bb-message-1",
            handle: { address: userAddress, displayName: "Blue User" },
      ...
      FLYFLOR_GATEWAY_CHANNELS: "bluebubbles",
  ...
  await waitFor(
    () => bluebubblesRequests.find((request) => request.method === "POST"),
    15_000,
    "bluebubbles message/text POST"
  );
...
function buildReport(): Report {
  ...
  if (objectField(inbound, "payload.text") !== "hello from bluebubbles") {
    failedChecks.push("gateway.message.send text mismatch");
  }
  if (objectField(inbound, "payload.conversationKey") !== `bluebubbles:${chatGuid}`) {
    failedChecks.push("gateway.message.send conversationKey mismatch");
  }
  if (objectField(inbound, "payload.metadata.channel.platform") !== "bluebubbles") {
    failedChecks.push("gateway.message.send metadata platform mismatch");
  }
  if (objectField(inbound, "payload.metadata.channel.sourceMessageId") !== "bb-message-1") {
    failedChecks.push("gateway.message.send sourceMessageId missing");
  }
  const post = bluebubblesRequests.find((request) => request.method === "POST");
  if (!post) failedChecks.push("mock BlueBubbles did not receive message text POST");
  if (!post?.url.startsWith("/api/v1/message/text?")) {
    failedChecks.push("BlueBubbles text endpoint mismatch");
  }
  const url = new URL(`http://127.0.0.1${post?.url ?? "/"}`);
  if (url.searchParams.get("password") !== password) {
    failedChecks.push("BlueBubbles password query mismatch");
  }
  const body = parseJson(post?.body ?? "");
  if (objectField(body, "chatGuid") !== chatGuid) failedChecks.push("BlueBubbles chatGuid mismatch");
  if (objectField(body, "message") !== "bluebubbles reply") failedChecks.push("BlueBubbles message mismatch");
  if (objectField(body, "method") !== "apple-script") failedChecks.push("BlueBubbles method mismatch");
...
async function startBlueBubblesServer(bluebubblesRequests: BlueBubblesRequestRecord[]) {
  const server = createServer(async (request, response) => {
    const body = await readBody(request);
    bluebubblesRequests.push({
      body,
      headers: request.headers,
      method: request.method ?? "GET",
      url: request.url ?? "",
    });
    if (request.method === "POST" && request.url?.startsWith("/api/v1/message/text?")) {
      ...
      response.end(JSON.stringify({ data: { guid: "bb-reply-1" }, message: "Message sent!" }));
      return;
    }
  ...
  if (!address || typeof address === "string") throw new Error("BlueBubbles server did not bind a TCP port");

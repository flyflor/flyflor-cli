...
interface Report {
  ...
  ok: boolean;
  outputDir: string;
  signalRequests: SignalRequestRecord[];
}
...
const tempRoot = mkdtempSync(join(tmpdir(), "flyflor-signal-smoke-"));
const cliHome = resolve(tempRoot, "cli-home");
const gatewayLogPath = resolve(cliHome, "logs", "gateway.log");
const account = "+15550000";
const sender = "+15550001";
const sourceMessageId = "1779958000000";
const outputDir = resolve(root, ".flyflor-cli", "signal-smoke", timestamp());

mkdirSync(outputDir, { recursive: true });

const kernelMessages: unknown[] = [];
const signalRequests: SignalRequestRecord[] = [];
let gateway: ReturnType<typeof spawn> | undefined;
let cleanup: (() => void) | undefined;

try {
  const signal = await startSignalServer(signalRequests);
  ...
  gateway = spawn("cargo", ["run", "--quiet", "--", "gateway", "run"], {
    ...
    env: {
      ...
      FLYFLOR_GATEWAY_CHANNELS: "signal",
      ...
      SIGNAL_ALLOWED_USERS: sender,
      SIGNAL_CLI_REST_API: `http://127.0.0.1:${signal.port}`,
      SIGNAL_INBOUND_ENVELOPE: JSON.stringify({
        envelope: {
          dataMessage: { message: "hello from signal" },
          sourceName: "Signal User",
          sourceNumber: sender,
          timestamp: Number(sourceMessageId),
        },
      }),
      SIGNAL_PHONE_NUMBER: account,
    },
  ...
  await waitFor(
    () => signalRequests.find((request) => request.method === "POST" && request.url === "/api/v1/rpc"),
    15_000,
    "signal JSON-RPC POST"
  );
...
function buildReport(): Report {
  ...
  if (objectField(inbound, "payload.text") !== "hello from signal") {
    failedChecks.push("gateway.message.send text mismatch");
  }
  if (objectField(inbound, "payload.conversationKey") !== `signal:${sender}`) {
    failedChecks.push("gateway.message.send conversationKey mismatch");
  }
  if (objectField(inbound, "payload.metadata.channel.platform") !== "signal") {
  ...
  const post = signalRequests.find((request) => request.method === "POST" && request.url === "/api/v1/rpc");
  if (!post) failedChecks.push("mock signal-cli did not receive JSON-RPC POST");
  const postBody = parseJson(post?.body ?? "");
  if (objectField(postBody, "method") !== "send") failedChecks.push("Signal RPC method mismatch");
  if (objectField(postBody, "params.account") !== account) failedChecks.push("Signal account mismatch");
  if (objectField(postBody, "params.message") !== "signal reply") {
    failedChecks.push("Signal reply message mismatch");
  }
  if (JSON.stringify(objectField(postBody, "params.recipient")) !== JSON.stringify([sender])) {
    failedChecks.push("Signal recipient mismatch");
  }
...
async function startSignalServer(signalRequests: SignalRequestRecord[]) {
  ...
  if (!address || typeof address === "string") throw new Error("Signal server did not bind a TCP port");

...
interface Report {
  ...
  ok: boolean;
  outputDir: string;
  twilioRequests: TwilioRequestRecord[];
}

interface TwilioRequestRecord {
  body: string;
...
const tempRoot = mkdtempSync(join(tmpdir(), "flyflor-sms-smoke-"));
const cliHome = resolve(tempRoot, "cli-home");
const gatewayLogPath = resolve(cliHome, "logs", "gateway.log");
const accountSid = "ACsmoke";
const fromNumber = "+15550000";
const userNumber = "+15550001";
const outputDir = resolve(root, ".flyflor-cli", "sms-smoke", timestamp());

mkdirSync(outputDir, { recursive: true });

const kernelMessages: unknown[] = [];
const twilioRequests: TwilioRequestRecord[] = [];
let gateway: ReturnType<typeof spawn> | undefined;
let cleanup: (() => void) | undefined;

try {
  const twilio = await startTwilioServer(twilioRequests);
  ...
  gateway = spawn("cargo", ["run", "--quiet", "--", "gateway", "run"], {
    ...
    env: {
      ...
      SMS_ALLOWED_USERS: userNumber,
      SMS_INBOUND_WEBHOOK: `MessageSid=SMsmoke&From=${encodeURIComponent(userNumber)}&To=${encodeURIComponent(fromNumber)}&Body=hello+from+sms`,
      TWILIO_ACCOUNT_SID: accountSid,
      TWILIO_API_BASE: `http://127.0.0.1:${twilio.port}`,
  ...
  await waitFor(
    () => twilioRequests.find((request) => request.method === "POST"),
    15_000,
...
function buildReport(): Report {
  ...
  if (objectField(inbound, "payload.text") !== "hello from sms") {
    failedChecks.push("gateway.message.send text mismatch");
  }
  if (objectField(inbound, "payload.conversationKey") !== `sms:${userNumber}`) {
    failedChecks.push("gateway.message.send conversationKey mismatch");
  }
  if (objectField(inbound, "payload.metadata.channel.platform") !== "sms") {
    failedChecks.push("gateway.message.send metadata platform mismatch");
  }
  const send = twilioRequests.find((request) => request.method === "POST");
  if (!send) failedChecks.push("mock twilio did not receive messages POST");
  if (send?.url !== `/2010-04-01/Accounts/${accountSid}/Messages.json`) {
    failedChecks.push("twilio messages path mismatch");
  }
  const body = new URLSearchParams(send?.body ?? "");
  if (body.get("From") !== fromNumber) failedChecks.push("twilio From mismatch");
  if (body.get("To") !== userNumber) failedChecks.push("twilio To mismatch");
  if (body.get("Body") !== "sms reply") failedChecks.push("twilio Body mismatch");
...
async function startTwilioServer(twilioRequests: TwilioRequestRecord[]) {
  const server = createServer(async (request, response) => {
    ...
    if (request.method === "POST" && request.url === `/2010-04-01/Accounts/${accountSid}/Messages.json`) {
      response.writeHead(200, { "content-type": "application/json" });
      response.end(JSON.stringify({ sid: "SMreply", status: "queued" }));
      return;
    }
  ...
  if (!address || typeof address === "string") throw new Error("twilio server did not bind a TCP port");

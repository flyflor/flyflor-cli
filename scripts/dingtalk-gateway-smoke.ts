...
interface Report {
  dingtalkRequests: DingTalkRequestRecord[];
  failedChecks: string[];
...
interface DingTalkRequestRecord {
  body: string;
...
const tempRoot = mkdtempSync(join(tmpdir(), "flyflor-dingtalk-smoke-"));
const cliHome = resolve(tempRoot, "cli-home");
const gatewayLogPath = resolve(cliHome, "logs", "gateway.log");
const conversationId = "cid-smoke";
const messageId = "msg-smoke";
const userId = "staff-1";
const outputDir = resolve(root, ".flyflor-cli", "dingtalk-smoke", timestamp());

mkdirSync(outputDir, { recursive: true });

const dingtalkRequests: DingTalkRequestRecord[] = [];
const kernelMessages: unknown[] = [];
let gateway: ReturnType<typeof spawn> | undefined;
let cleanup: (() => void) | undefined;

try {
  const dingtalk = await startDingTalkServer(dingtalkRequests);
  const kernel = await startKernelServer(kernelMessages);
  cleanup = () => {
    kernel.close();
    dingtalk.server.close();
  };
  gateway = spawn("cargo", ["run", "--quiet", "--", "gateway", "run"], {
    cwd: root,
    env: {
      ...process.env,
      DINGTALK_ALLOWED_USERS: userId,
      DINGTALK_API_BASE: `http://127.0.0.1:${dingtalk.port}`,
      DINGTALK_CLIENT_ID: "client-id",
      DINGTALK_CLIENT_SECRET: "client-secret",
      DINGTALK_INBOUND_WEBHOOK: JSON.stringify({
        conversationId,
        msgId: messageId,
        senderNick: "DingTalk User",
        senderStaffId: userId,
        sessionWebhook: `http://127.0.0.1:${dingtalk.port}/session-webhook`,
        text: { content: "hello from dingtalk" },
      }),
      FLYFLOR_CLI_HOME: cliHome,
      FLYFLOR_GATEWAY_CHANNELS: "dingtalk",
  ...
  await waitFor(
    () => dingtalkRequests.find((request) => request.method === "POST" && request.url === "/session-webhook"),
    15_000,
    "dingtalk session webhook POST"
  );
...
function buildReport(): Report {
  ...
  if (objectField(inbound, "payload.text") !== "hello from dingtalk") {
    failedChecks.push("gateway.message.send text mismatch");
  }
  if (objectField(inbound, "payload.conversationKey") !== `dingtalk:${conversationId}`) {
    failedChecks.push("gateway.message.send conversationKey mismatch");
  }
  if (objectField(inbound, "payload.metadata.channel.platform") !== "dingtalk") {
  ...
  if (String(objectField(inbound, "payload.metadata.channel.sessionWebhook") ?? "").endsWith("/session-webhook") === false) {
    failedChecks.push("gateway.message.send sessionWebhook metadata mismatch");
  }
  const post = dingtalkRequests.find((request) => request.method === "POST" && request.url === "/session-webhook");
  if (!post) failedChecks.push("mock DingTalk did not receive session webhook POST");
  const postBody = parseJson(post?.body ?? "");
  if (objectField(postBody, "msgtype") !== "text") failedChecks.push("DingTalk msgtype mismatch");
  if (objectField(postBody, "text.content") !== "dingtalk reply") {
    failedChecks.push("DingTalk reply content mismatch");
  }
...
async function startDingTalkServer(dingtalkRequests: DingTalkRequestRecord[]) {
  const server = createServer(async (request, response) => {
    const body = await readBody(request);
    dingtalkRequests.push({
      body,
      headers: request.headers,
      method: request.method ?? "GET",
      url: request.url ?? "",
    });
    if (request.method === "POST" && request.url === "/v1.0/oauth2/accessToken") {
      response.writeHead(200, { "content-type": "application/json" });
      response.end(JSON.stringify({ accessToken: "dingtalk-token" }));
      return;
    }
    if (request.method === "POST" && request.url === "/session-webhook") {
      response.writeHead(200, { "content-type": "application/json" });
      response.end(JSON.stringify({ errcode: 0, messageId: "reply-message" }));
      return;
    }
    if (request.method === "POST" && request.url === "/v1.0/robot/groupMessages/send") {
      response.writeHead(200, { "content-type": "application/json" });
      response.end(JSON.stringify({ processQueryKey: "robot-process" }));
      return;
    }
  ...
  if (!address || typeof address === "string") throw new Error("DingTalk server did not bind a TCP port");

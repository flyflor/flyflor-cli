...
interface Report {
  ...
  ok: boolean;
  outputDir: string;
  wecomRequests: WeComRequestRecord[];
}

interface WeComRequestRecord {
  body: string;
...
const tempRoot = mkdtempSync(join(tmpdir(), "flyflor-wecom-callback-smoke-"));
...
const outputDir = resolve(root, ".flyflor-cli", "wecom-callback-smoke", timestamp());

mkdirSync(outputDir, { recursive: true });

const kernelMessages: unknown[] = [];
const wecomRequests: WeComRequestRecord[] = [];
let gateway: ReturnType<typeof spawn> | undefined;
let cleanup: (() => void) | undefined;

try {
  const wecom = await startWeComServer(wecomRequests);
  ...
  gateway = spawn("cargo", ["run", "--quiet", "--", "gateway", "run"], {
    ...
    env: {
      ...
      FLYFLOR_GATEWAY_CHANNELS: "wecom-callback",
      ...
      WECOM_API_BASE: `http://127.0.0.1:${wecom.port}`,
      WECOM_CALLBACK_AES_KEY: "abcdefghijklmnopqrstuvwxyz0123456789ABCDEFG",
      WECOM_CALLBACK_ALLOWED_USERS: userId,
      WECOM_CALLBACK_INBOUND_EVENT: JSON.stringify({
  ...
  await waitFor(
    () => wecomRequests.find((request) => request.method === "POST" && request.url.startsWith("/message/send")),
    15_000,
    "wecom message/send POST"
  );
...
function buildReport(): Report {
  ...
  if (objectField(inbound, "payload.conversationKey") !== `wecom-callback:${corpId}:${userId}`) {
    failedChecks.push("gateway.message.send conversationKey mismatch");
  }
  if (objectField(inbound, "payload.metadata.channel.platform") !== "wecom-callback") {
  ...
  if (objectField(inbound, "payload.metadata.channel.corpId") !== corpId) {
    failedChecks.push("gateway.message.send corpId mismatch");
  }
  const token = wecomRequests.find((request) => request.method === "GET" && request.url.startsWith("/gettoken"));
  if (!token) failedChecks.push("mock WeCom did not receive gettoken GET");
  if (!token?.url.includes(`corpid=${corpId}`) || !token.url.includes(`corpsecret=${corpSecret}`)) {
    failedChecks.push("WeCom gettoken query mismatch");
  }
  const post = wecomRequests.find((request) => request.method === "POST" && request.url.startsWith("/message/send"));
  if (!post) failedChecks.push("mock WeCom did not receive message/send POST");
  if (!post?.url.includes("access_token=wecom-token")) {
    failedChecks.push("WeCom message/send token mismatch");
  }
  const postBody = parseJson(post?.body ?? "");
  if (objectField(postBody, "touser") !== userId) failedChecks.push("WeCom touser mismatch");
  if (objectField(postBody, "msgtype") !== "text") failedChecks.push("WeCom msgtype mismatch");
  if (objectField(postBody, "agentid") !== Number(agentId)) failedChecks.push("WeCom agentid mismatch");
  if (objectField(postBody, "text.content") !== "wecom reply") {
...
async function startWeComServer(wecomRequests: WeComRequestRecord[]) {
  const server = createServer(async (request, response) => {
    ...
    if (request.method === "GET" && request.url?.startsWith("/gettoken")) {
      response.writeHead(200, { "content-type": "application/json" });
      response.end(JSON.stringify({ access_token: "wecom-token", errcode: 0, expires_in: 7200 }));
      return;
    }
    if (request.method === "POST" && request.url?.startsWith("/message/send")) {
      response.writeHead(200, { "content-type": "application/json" });
      response.end(JSON.stringify({ errcode: 0, errmsg: "ok", msgid: "reply-message" }));
      return;
    }

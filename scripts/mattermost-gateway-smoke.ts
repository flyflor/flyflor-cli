...
interface Report {
  ...
  mattermostRequests: MattermostRequestRecord[];
  ok: boolean;
  outputDir: string;
}

interface MattermostRequestRecord {
  body: string;
...
const tempRoot = mkdtempSync(join(tmpdir(), "flyflor-mattermost-smoke-"));
const cliHome = resolve(tempRoot, "cli-home");
const gatewayLogPath = resolve(cliHome, "logs", "gateway.log");
const channelId = "channel-1";
const outputDir = resolve(root, ".flyflor-cli", "mattermost-smoke", timestamp());

mkdirSync(outputDir, { recursive: true });

const kernelMessages: unknown[] = [];
const mattermostRequests: MattermostRequestRecord[] = [];
let gateway: ReturnType<typeof spawn> | undefined;
let cleanup: (() => void) | undefined;

try {
  const mattermost = await startMattermostServer(mattermostRequests);
  const kernel = await startKernelServer(kernelMessages);
  cleanup = () => {
    kernel.close();
    mattermost.server.close();
  };
  gateway = spawn("cargo", ["run", "--quiet", "--", "gateway", "run"], {
    ...
    env: {
      ...
      FLYFLOR_GATEWAY_CHANNELS: "mattermost",
      ...
      MATTERMOST_ALLOWED_USERS: "user-1",
      MATTERMOST_CHANNEL: channelId,
      MATTERMOST_TOKEN: "smoke-token",
      MATTERMOST_URL: `http://127.0.0.1:${mattermost.port}`,
    },
  ...
  await waitFor(
    () => mattermostRequests.find((request) => request.method === "POST" && request.url === "/api/v4/posts"),
    15_000,
    "mattermost create post"
  );
...
function buildReport(): Report {
  ...
  if (objectField(inbound, "payload.text") !== "hello from mattermost") {
    failedChecks.push("gateway.message.send text mismatch");
  }
  if (objectField(inbound, "payload.conversationKey") !== `mattermost:${channelId}`) {
    failedChecks.push("gateway.message.send conversationKey mismatch");
  }
  if (objectField(inbound, "payload.metadata.channel.platform") !== "mattermost") {
    failedChecks.push("gateway.message.send metadata platform mismatch");
  }
  const poll = mattermostRequests.find((request) => request.method === "GET" && request.url.startsWith(`/api/v4/channels/${channelId}/posts`));
  if (!poll) failedChecks.push("mock mattermost did not receive posts GET");
  const post = mattermostRequests.find((request) => request.method === "POST" && request.url === "/api/v4/posts");
  if (!post) failedChecks.push("mock mattermost did not receive create post");
  if (post && objectField(parseJson(post.body), "channel_id") !== channelId) {
    failedChecks.push("mattermost post channel mismatch");
  }
  if (post && objectField(parseJson(post.body), "message") !== "mattermost reply") {
    failedChecks.push("mattermost post message mismatch");
  }
...
async function startMattermostServer(mattermostRequests: MattermostRequestRecord[]) {
  let delivered = false;
  const server = createServer(async (request, response) => {
    const body = await readBody(request);
    mattermostRequests.push({
      body,
      headers: request.headers,
      method: request.method ?? "GET",
      url: request.url ?? "",
    });
    if (request.method === "GET" && request.url?.startsWith(`/api/v4/channels/${channelId}/posts`)) {
          ...
          JSON.stringify({
            ...
            posts: {
              "post-1": {
                ...
                create_at: 1770000000000,
              },
            },
          })
        );
      } else {
        response.end(JSON.stringify({ order: [], posts: {} }));
      }
      return;
    }
    if (request.method === "POST" && request.url === "/api/v4/posts") {
      response.writeHead(200, { "content-type": "application/json" });
      response.end(JSON.stringify({ id: "reply-1", channel_id: channelId }));
      return;
    }
  ...
  if (!address || typeof address === "string") throw new Error("mattermost server did not bind a TCP port");

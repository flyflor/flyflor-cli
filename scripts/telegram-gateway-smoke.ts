...
interface Report {
  ...
  ok: boolean;
  outputDir: string;
  telegramRequests: RequestRecord[];
}
...
const tempRoot = mkdtempSync(join(tmpdir(), "flyflor-telegram-smoke-"));
...
const messageId = 42;
const updateId = 1001;
const outputDir = resolve(root, ".flyflor-cli", "telegram-smoke", timestamp());

mkdirSync(outputDir, { recursive: true });

const kernelMessages: unknown[] = [];
const telegramRequests: RequestRecord[] = [];
let gateway: ReturnType<typeof spawn> | undefined;
let cleanup: (() => void) | undefined;

try {
  const telegram = await startTelegramApiServer(telegramRequests);
  ...
  gateway = spawn("cargo", ["run", "--quiet", "--", "gateway", "run"], {
    ...
    env: {
      ...
      FLYFLOR_GATEWAY_CHANNELS: "telegram",
      ...
      TELEGRAM_ALLOWED_USERS: String(userId),
      TELEGRAM_BASE_URL: `http://127.0.0.1:${telegram.port}`,
  ...
  await waitFor(
    () => telegramRequests.find((request) => request.method === "POST" && request.url === `/bot${token}/sendMessage`),
    15_000,
...
function buildReport(): Report {
  ...
  if (objectField(inbound, "payload.text") !== "hello from telegram") {
    failedChecks.push("gateway.message.send text mismatch");
  }
  if (objectField(inbound, "payload.conversationKey") !== `telegram:${chatId}`) {
    failedChecks.push("gateway.message.send conversationKey mismatch");
  }
  if (objectField(inbound, "payload.metadata.channel.platform") !== "telegram") {
    failedChecks.push("gateway.message.send metadata platform mismatch");
  }
  if (objectField(inbound, "payload.metadata.channel.sourceMessageId") !== `${chatId}:${messageId}`) {
    failedChecks.push("gateway.message.send source message mismatch");
  }
  if (objectField(inbound, "payload.metadata.channel.updateId") !== updateId) {
    failedChecks.push("gateway.message.send update id mismatch");
  }
  const post = telegramRequests.find((request) => request.method === "POST" && request.url === `/bot${token}/sendMessage`);
  if (!post) failedChecks.push("mock Telegram API did not receive sendMessage");
  const body = parseJson(post?.body ?? "");
  if (objectField(body, "chat_id") !== String(chatId)) {
    failedChecks.push("Telegram sendMessage chat_id mismatch");
  }
  if (objectField(body, "text") !== "telegram reply") {
    failedChecks.push("Telegram sendMessage text mismatch");
  }
...
async function startTelegramApiServer(records: RequestRecord[]) {
  let delivered = false;
  const server = createServer(async (request, response) => {
    ...
    if (request.method === "POST" && request.url === `/bot${token}/getUpdates`) {
        ...
        : [
            {
              ...
              message: {
                ...
                from: { id: userId, first_name: "Ada", username: "ada" },
    ...
    if (request.method === "POST" && request.url === `/bot${token}/sendMessage`) {
      response.writeHead(200, { "content-type": "application/json" });
      response.end(JSON.stringify({ ok: true, result: { message_id: 9001 } }));
      return;
    }
    if (request.method === "POST" && request.url === `/bot${token}/sendChatAction`) {
      response.writeHead(200, { "content-type": "application/json" });
      response.end(JSON.stringify({ ok: true, result: true }));
      return;
    }
    if (request.method === "POST" && request.url === `/bot${token}/editMessageText`) {
      response.writeHead(200, { "content-type": "application/json" });
      response.end(JSON.stringify({ ok: true, result: { message_id: 9001 } }));
      return;
    }
  ...
  if (!address || typeof address === "string") throw new Error("Telegram server did not bind a TCP port");

...
interface Report {
  ...
  lineRequests: LineRequestRecord[];
  ok: boolean;
  outputDir: string;
}

interface LineRequestRecord {
  body: string;
...
const tempRoot = mkdtempSync(join(tmpdir(), "flyflor-line-smoke-"));
const cliHome = resolve(tempRoot, "cli-home");
const gatewayLogPath = resolve(cliHome, "logs", "gateway.log");
const outputDir = resolve(root, ".flyflor-cli", "line-smoke", timestamp());
const userId = "line-user-1";

mkdirSync(outputDir, { recursive: true });

const kernelMessages: unknown[] = [];
const lineRequests: LineRequestRecord[] = [];
let gateway: ReturnType<typeof spawn> | undefined;
let cleanup: (() => void) | undefined;

try {
  const line = await startLineServer(lineRequests);
  ...
  gateway = spawn("cargo", ["run", "--quiet", "--", "gateway", "run"], {
    ...
    env: {
      ...
      LINE_ALLOWED_USERS: userId,
      LINE_API_BASE: `http://127.0.0.1:${line.port}`,
      LINE_CHANNEL_ACCESS_TOKEN: "line-token",
      LINE_CHANNEL_SECRET: "line-secret",
      LINE_INBOUND_WEBHOOK: JSON.stringify({
        events: [
          {
            message: { id: "line-message-1", text: "hello from line", type: "text" },
  ...
  await waitFor(
    () => lineRequests.find((request) => request.method === "POST"),
    15_000,
    "line reply POST"
  );
...
function buildReport(): Report {
  ...
  if (objectField(inbound, "payload.text") !== "hello from line") {
    failedChecks.push("gateway.message.send text mismatch");
  }
  if (objectField(inbound, "payload.conversationKey") !== `line:${userId}`) {
    failedChecks.push("gateway.message.send conversationKey mismatch");
  }
  if (objectField(inbound, "payload.metadata.channel.platform") !== "line") {
    failedChecks.push("gateway.message.send metadata platform mismatch");
  }
  if (objectField(inbound, "payload.metadata.channel.replyToken") !== "reply-token-1") {
    failedChecks.push("gateway.message.send reply token missing");
  }
  const reply = lineRequests.find((request) => request.method === "POST" && request.url === "/v2/bot/message/reply");
  if (!reply) failedChecks.push("mock LINE did not receive reply POST");
  const body = parseJson(reply?.body ?? "");
  if (objectField(body, "replyToken") !== "reply-token-1") {
    failedChecks.push("LINE reply token mismatch");
  }
  if (objectField(body, "messages.0.text") !== "line reply") {
    failedChecks.push("LINE reply text mismatch");
  }
...
async function startLineServer(lineRequests: LineRequestRecord[]) {
  const server = createServer(async (request, response) => {
    ...
    if (request.method === "POST" && request.url === "/v2/bot/message/reply") {
      response.writeHead(200, { "content-type": "application/json" });
      response.end(JSON.stringify({ messageId: "line-reply-1" }));
      return;
    }
    if (request.method === "POST" && request.url === "/v2/bot/message/push") {
      response.writeHead(200, { "content-type": "application/json" });
      response.end(JSON.stringify({ messageId: "line-push-1" }));
      return;
    }
  ...
  if (!address || typeof address === "string") throw new Error("LINE server did not bind a TCP port");

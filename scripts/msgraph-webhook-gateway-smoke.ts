...
const tempRoot = mkdtempSync(join(tmpdir(), "flyflor-msgraph-smoke-"));
const cliHome = resolve(tempRoot, "cli-home");
const gatewayLogPath = resolve(cliHome, "logs", "gateway.log");
const subscriptionId = "sub-1";
const notificationId = "notif-smoke";
const clientState = "expected-client-state";
const resource = "communications/onlineMeetings/meeting-1";
const outputDir = resolve(root, ".flyflor-cli", "msgraph-webhook-smoke", timestamp());

mkdirSync(outputDir, { recursive: true });

const kernelMessages: unknown[] = [];
const msgraphRequests: RequestRecord[] = [];
let gateway: ReturnType<typeof spawn> | undefined;
let cleanup: (() => void) | undefined;

try {
  const replyServer = await startReplyServer(msgraphRequests);
  const kernel = await startKernelServer(kernelMessages);
  cleanup = () => {
    kernel.close();
    replyServer.server.close();
  };
  gateway = spawn("cargo", ["run", "--quiet", "--", "gateway", "run"], {
    ...
    env: {
      ...
      FLYFLOR_GATEWAY_CHANNELS: "msgraph-webhook",
      ...
      MSGRAPH_ACCEPTED_RESOURCES: "communications/onlineMeetings",
      MSGRAPH_CLIENT_ID: "client-1",
      MSGRAPH_CLIENT_SECRET: "secret",
      MSGRAPH_REPLY_WEBHOOK_URL: `http://127.0.0.1:${replyServer.port}/reply`,
      MSGRAPH_TENANT_ID: "tenant-1",
      MSGRAPH_WEBHOOK_INBOUND_EVENT: JSON.stringify({
        value: [
          {
            changeType: "updated",
            clientState,
      ...
      MSGRAPH_WEBHOOK_SECRET: clientState,
    },
  ...
  await waitFor(
    () => msgraphRequests.find((request) => request.method === "POST" && request.url === "/reply"),
    15_000,
    "MSGraph reply webhook POST"
  );
...
function buildReport(): Report {
  ...
  if (String(objectField(inbound, "payload.text") ?? "").includes(resource) === false) {
    failedChecks.push("gateway.message.send text does not include resource");
  }
  if (objectField(inbound, "payload.conversationKey") !== `msgraph-webhook:msgraph:${subscriptionId}`) {
    failedChecks.push("gateway.message.send conversationKey mismatch");
  }
  if (objectField(inbound, "payload.metadata.channel.platform") !== "msgraph-webhook") {
    failedChecks.push("gateway.message.send metadata platform mismatch");
  }
  if (objectField(inbound, "payload.metadata.channel.sourceMessageId") !== `id:${notificationId}`) {
    failedChecks.push("gateway.message.send source message mismatch");
  }
  const post = msgraphRequests.find((request) => request.method === "POST" && request.url === "/reply");
  if (!post) failedChecks.push("mock MSGraph reply endpoint did not receive POST");
  const postBody = parseJson(post?.body ?? "");
  if (objectField(postBody, "text") !== "msgraph reply") {
    failedChecks.push("MSGraph reply content mismatch");
  }
  if (objectField(postBody, "route.chatId") !== `msgraph:${subscriptionId}`) {
    failedChecks.push("MSGraph reply route mismatch");
  }
...
async function startReplyServer(records: RequestRecord[]) {
  const server = createServer(async (request, response) => {
    ...
    if (request.method === "POST" && request.url === "/reply") {
      ...
      response.end(JSON.stringify({ id: "reply-message" }));
      return;
    }
  ...
  if (!address || typeof address === "string") throw new Error("reply server did not bind a TCP port");

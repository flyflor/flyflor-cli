...
interface Report {
  discordRequests: DiscordRequestRecord[];
  failedChecks: string[];
...
const tempRoot = mkdtempSync(join(tmpdir(), "flyflor-discord-smoke-"));
const cliHome = resolve(tempRoot, "cli-home");
const gatewayLogPath = resolve(cliHome, "logs", "gateway.log");
const channelId = "discord-channel-1";
const userId = "discord-user-1";
const outputDir = resolve(root, ".flyflor-cli", "discord-smoke", timestamp());
...
try {
  const discord = await startDiscordServer(discordRequests);
  ...
  gateway = spawn("cargo", ["run", "--quiet", "--", "gateway", "run"], {
    ...
    env: {
      ...
      DISCORD_API_BASE: `http://127.0.0.1:${discord.port}`,
      DISCORD_BOT_TOKEN: "discord-token",
      DISCORD_BOT_USER_ID: "bot-user-1",
      DISCORD_HOME_CHANNEL: channelId,
      FLYFLOR_CLI_HOME: cliHome,
      FLYFLOR_GATEWAY_CHANNELS: "discord",
  ...
  await waitFor(
    () => discordRequests.find((request) => request.method === "POST" && request.url === `/api/v10/channels/${channelId}/messages`),
    15_000,
...
function buildReport(): Report {
  ...
  if (objectField(inbound, "payload.text") !== "hello from discord") {
    failedChecks.push("gateway.message.send text mismatch");
  }
  if (objectField(inbound, "payload.conversationKey") !== `discord:${channelId}`) {
    failedChecks.push("gateway.message.send conversationKey mismatch");
  }
  if (objectField(inbound, "payload.metadata.channel.platform") !== "discord") {
    failedChecks.push("gateway.message.send metadata platform mismatch");
  }
  const poll = discordRequests.find((request) => request.method === "GET" && request.url.startsWith(`/api/v10/channels/${channelId}/messages`));
  if (!poll) failedChecks.push("mock Discord did not receive messages GET");
  const post = discordRequests.find((request) => request.method === "POST" && request.url === `/api/v10/channels/${channelId}/messages`);
  if (!post) failedChecks.push("mock Discord did not receive create message");
  const postBody = parseJson(post?.body ?? "");
  if (objectField(postBody, "content") !== "discord reply") failedChecks.push("Discord post content mismatch");
  if (objectField(postBody, "allowed_mentions.parse") && Array.isArray(objectField(postBody, "allowed_mentions.parse")) === false) {
    failedChecks.push("Discord allowed_mentions parse mismatch");
  }
  if (objectField(postBody, "message_reference.message_id") !== "discord-message-1") {
    failedChecks.push("Discord message reference mismatch");
  }
...
async function startDiscordServer(discordRequests: DiscordRequestRecord[]) {
  let delivered = false;
  const server = createServer(async (request, response) => {
    ...
    if (request.method === "GET" && request.url?.startsWith(`/api/v10/channels/${channelId}/messages`)) {
      ...
      if (!delivered) {
        ...
        response.end(
          JSON.stringify([
            {
              author: { bot: false, id: userId, username: "Discord User" },
    ...
    if (request.method === "POST" && request.url === `/api/v10/channels/${channelId}/messages`) {
      response.writeHead(200, { "content-type": "application/json" });
      response.end(JSON.stringify({ id: "discord-reply-1", channel_id: channelId }));
      return;
    }
  ...
  if (!address || typeof address === "string") throw new Error("Discord server did not bind a TCP port");

...
const tempRoot = mkdtempSync(join(tmpdir(), "flyflor-yuanbao-smoke-"));
const cliHome = resolve(tempRoot, "cli-home");
const gatewayLogPath = resolve(cliHome, "logs", "gateway.log");
const appId = "app-1";
const botId = "bot-1";
const groupCode = "group-1";
const fromAccount = "user-1";
const messageId = "yb-msg-smoke";
const outputDir = resolve(root, ".flyflor-cli", "yuanbao-smoke", timestamp());

mkdirSync(outputDir, { recursive: true });

const kernelMessages: unknown[] = [];
const yuanbaoRequests: RequestRecord[] = [];
let gateway: ReturnType<typeof spawn> | undefined;
let cleanup: (() => void) | undefined;

try {
  const yuanbao = await startYuanbaoReplyServer(yuanbaoRequests);
  const kernel = await startKernelServer(kernelMessages);
  cleanup = () => {
    kernel.close();
    yuanbao.server.close();
  };
  gateway = spawn("cargo", ["run", "--quiet", "--", "gateway", "run"], {
    ...
    env: {
      ...
      FLYFLOR_GATEWAY_CHANNELS: "yuanbao",
      ...
      YUANBAO_APP_ID: appId,
      YUANBAO_APP_SECRET: "secret",
      YUANBAO_BOT_ID: botId,
      YUANBAO_GROUP_ALLOW_FROM: groupCode,
      YUANBAO_GROUP_POLICY: "allowlist",
      YUANBAO_INBOUND_EVENT: JSON.stringify({
        ...
        log_ext: { trace_id: "trace-smoke" },
      }),
      YUANBAO_REPLY_WEBHOOK_URL: `http://127.0.0.1:${yuanbao.port}/reply`,
    },
  ...
  await waitFor(
    () => yuanbaoRequests.find((request) => request.method === "POST" && request.url === "/reply"),
    15_000,
    "Yuanbao reply webhook POST"
  );
...
function buildReport(): Report {
  ...
  if (objectField(inbound, "payload.text") !== "hello yuanbao") {
    failedChecks.push("gateway.message.send text mismatch");
  }
  if (objectField(inbound, "payload.conversationKey") !== `yuanbao:group:${groupCode}`) {
    failedChecks.push("gateway.message.send conversationKey mismatch");
  }
  if (objectField(inbound, "payload.metadata.channel.platform") !== "yuanbao") {
  ...
  if (objectField(inbound, "payload.metadata.channel.traceId") !== "trace-smoke") {
    failedChecks.push("gateway.message.send trace id mismatch");
  }
  const post = yuanbaoRequests.find((request) => request.method === "POST" && request.url === "/reply");
  if (!post) failedChecks.push("mock Yuanbao reply webhook did not receive POST");
  const body = parseJson(post?.body ?? "");
  if (objectField(body, "text") !== "yuanbao reply") {
    failedChecks.push("Yuanbao reply webhook text mismatch");
  }
  if (objectField(body, "route.chatId") !== `group:${groupCode}`) {
    failedChecks.push("Yuanbao reply route mismatch");
  }
...
async function startYuanbaoReplyServer(records: RequestRecord[]) {
  const server = createServer(async (request, response) => {
    ...
    if (request.method === "POST" && request.url === "/reply") {
      ...
      response.end(JSON.stringify({ msg_id: "yb-reply-message" }));
      return;
    }
  ...
  if (!address || typeof address === "string") throw new Error("Yuanbao server did not bind a TCP port");

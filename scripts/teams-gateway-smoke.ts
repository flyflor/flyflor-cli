...
const tempRoot = mkdtempSync(join(tmpdir(), "flyflor-teams-smoke-"));
...
const activityId = "activity-smoke";
const aadUserId = "aad-user-1";
const teamId = "team-1";
const channelId = "channel-1";
const outputDir = resolve(root, ".flyflor-cli", "teams-smoke", timestamp());

mkdirSync(outputDir, { recursive: true });

const kernelMessages: unknown[] = [];
const teamsRequests: RequestRecord[] = [];
let gateway: ReturnType<typeof spawn> | undefined;
let cleanup: (() => void) | undefined;

try {
  const teams = await startTeamsWebhookServer(teamsRequests);
  ...
  gateway = spawn("cargo", ["run", "--quiet", "--", "gateway", "run"], {
    ...
    env: {
      ...
      FLYFLOR_GATEWAY_CHANNELS: "teams",
      ...
      TEAMS_ALLOWED_USERS: aadUserId,
      TEAMS_CLIENT_ID: clientId,
      TEAMS_CLIENT_SECRET: "secret",
      TEAMS_INBOUND_EVENT: JSON.stringify({
        type: "message",
        id: activityId,
        text: "<at>Flyflor</at> hello from teams",
        from: {
          id: "teams-user-1",
          aadObjectId: aadUserId,
          name: "Ada",
        },
        conversation: {
          id: conversationId,
      ...
      TEAMS_INCOMING_WEBHOOK_URL: `http://127.0.0.1:${teams.port}/incoming`,
  ...
  await waitFor(
    () => teamsRequests.find((request) => request.method === "POST" && request.url === "/incoming"),
    15_000,
...
function buildReport(): Report {
  ...
  if (objectField(inbound, "payload.text") !== "hello from teams") {
    failedChecks.push("gateway.message.send text mismatch");
  }
  if (objectField(inbound, "payload.conversationKey") !== `teams:${conversationId}`) {
    failedChecks.push("gateway.message.send conversationKey mismatch");
  }
  if (objectField(inbound, "payload.metadata.channel.platform") !== "teams") {
    failedChecks.push("gateway.message.send metadata platform mismatch");
  }
  if (objectField(inbound, "payload.metadata.channel.sourceMessageId") !== activityId) {
    failedChecks.push("gateway.message.send source message mismatch");
  }
  if (objectField(inbound, "payload.metadata.channel.teamId") !== teamId) {
    failedChecks.push("gateway.message.send team id mismatch");
  }
  if (objectField(inbound, "payload.metadata.channel.channelId") !== channelId) {
    failedChecks.push("gateway.message.send channel id mismatch");
  }
  const post = teamsRequests.find((request) => request.method === "POST" && request.url === "/incoming");
  if (!post) failedChecks.push("mock Teams incoming webhook did not receive POST");
  const body = parseJson(post?.body ?? "");
  if (objectField(body, "text") !== "teams reply") {
    failedChecks.push("Teams incoming webhook text mismatch");
  }
...
async function startTeamsWebhookServer(records: RequestRecord[]) {
  const server = createServer(async (request, response) => {
    ...
    if (request.method === "POST" && request.url === "/incoming") {
      ...
      response.end(JSON.stringify({ id: "teams-reply-message" }));
      return;
    }
    response.writeHead(404, { "content-type": "application/json" });
    response.end(JSON.stringify({ error: { code: "NotFound", message: "not found" } }));
  });
  await listen(server, 0);
  const address = server.address();
  if (!address || typeof address === "string") throw new Error("Teams server did not bind a TCP port");

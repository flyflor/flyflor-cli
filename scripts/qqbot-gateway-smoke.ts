...
interface Report {
  ...
  ok: boolean;
  outputDir: string;
  qqRequests: QqRequestRecord[];
}

interface QqRequestRecord {
  body: string;
...
try {
  ...
  gateway = spawn("cargo", ["run", "--quiet", "--", "gateway", "run"], {
    ...
    env: {
      ...
      FLYFLOR_GATEWAY_CHANNELS: "qqbot",
      ...
      QQBOT_ALLOWED_USERS: userId,
      QQBOT_API_BASE: `http://127.0.0.1:${qq.port}`,
      QQBOT_APP_ID: "app-id",
      QQBOT_INBOUND_EVENT: JSON.stringify({
        author: {
          member_openid: userId,
          username: "QQ User",
        },
        content: "@bot hello from qqbot",
        group_openid: groupOpenid,
        id: messageId,
      }),
      QQBOT_SECRET: "client-secret",
      QQBOT_TOKEN_URL: `http://127.0.0.1:${qq.port}/app/getAppAccessToken`,
    },
  ...
  await waitFor(
    () => qqRequests.find((request) => request.method === "POST" && request.url === `/v2/groups/${groupOpenid}/messages`),
    15_000,
    "qqbot group messages POST"
  );
...
function buildReport(): Report {
  ...
  if (objectField(inbound, "payload.text") !== "hello from qqbot") {
    failedChecks.push("gateway.message.send text mismatch");
  }
  if (objectField(inbound, "payload.conversationKey") !== `qqbot:${groupOpenid}`) {
    failedChecks.push("gateway.message.send conversationKey mismatch");
  }
  if (objectField(inbound, "payload.metadata.channel.platform") !== "qqbot") {
  ...
  if (objectField(inbound, "payload.metadata.channel.qqRoute") !== "group") {
    failedChecks.push("gateway.message.send qqRoute mismatch");
  }
  const token = qqRequests.find((request) => request.method === "POST" && request.url === "/app/getAppAccessToken");
  if (!token) failedChecks.push("mock QQ did not receive access token POST");
  const post = qqRequests.find((request) => request.method === "POST" && request.url === `/v2/groups/${groupOpenid}/messages`);
  if (!post) failedChecks.push("mock QQ did not receive group messages POST");
  if (post?.headers.authorization !== "QQBot qq-token") {
    failedChecks.push("QQBot authorization header mismatch");
  }
  const postBody = parseJson(post?.body ?? "");
  if (objectField(postBody, "content") !== "qqbot reply") {
    failedChecks.push("QQBot reply content mismatch");
  }
  if (objectField(postBody, "msg_type") !== 0) {
    failedChecks.push("QQBot msg_type mismatch");
  }
  if (objectField(postBody, "msg_id") !== "qqbot-msg-smoke") {
    failedChecks.push("QQBot reply anchor mismatch");
  }
...
async function startQqServer(qqRequests: QqRequestRecord[]) {
  const server = createServer(async (request, response) => {
    ...
    if (request.method === "POST" && request.url === "/app/getAppAccessToken") {
      response.writeHead(200, { "content-type": "application/json" });
      response.end(JSON.stringify({ access_token: "qq-token", expires_in: 7200 }));
      return;
    }
    if (request.method === "POST" && request.url === `/v2/groups/${groupOpenid}/messages`) {
      response.writeHead(200, { "content-type": "application/json" });
      response.end(JSON.stringify({ id: "qq-reply-message" }));
      return;
    }
  ...
  if (!address || typeof address === "string") throw new Error("QQ server did not bind a TCP port");

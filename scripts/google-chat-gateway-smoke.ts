...
interface Report {
  ...
  googleRequests: GoogleRequestRecord[];
  kernelMessages: unknown[];
  ok: boolean;
  outputDir: string;
}

interface GoogleRequestRecord {
  body: string;
...
const tempRoot = mkdtempSync(join(tmpdir(), "flyflor-google-chat-smoke-"));
const cliHome = resolve(tempRoot, "cli-home");
const gatewayLogPath = resolve(cliHome, "logs", "gateway.log");
const projectId = "project-1";
const subscriptionName = "projects/project-1/subscriptions/sub-1";
const spaceName = "spaces/S";
const threadName = "spaces/S/threads/T";
const messageName = "spaces/S/messages/M";
const userEmail = "user@example.com";
const outputDir = resolve(root, ".flyflor-cli", "google-chat-smoke", timestamp());

mkdirSync(outputDir, { recursive: true });

const googleRequests: GoogleRequestRecord[] = [];
const kernelMessages: unknown[] = [];
let gateway: ReturnType<typeof spawn> | undefined;
let cleanup: (() => void) | undefined;

try {
  const google = await startGoogleChatServer(googleRequests);
  ...
  gateway = spawn("cargo", ["run", "--quiet", "--", "gateway", "run"], {
    ...
    env: {
      ...
      FLYFLOR_GATEWAY_CHANNELS: "google-chat",
      ...
      GOOGLE_CHAT_ACCESS_TOKEN: "google-token",
      GOOGLE_CHAT_ALLOWED_USERS: userEmail,
      GOOGLE_CHAT_API_BASE: `http://127.0.0.1:${google.port}`,
      GOOGLE_CHAT_INBOUND_EVENT: JSON.stringify({
        chat: {
          messagePayload: {
            message: {
              argumentText: "hello from google chat",
              name: messageName,
              sender: {
              ...
              space: { name: spaceName, spaceType: "ROOM" },
      ...
      GOOGLE_CHAT_SUBSCRIPTION_NAME: subscriptionName,
    },
  ...
  await waitFor(
    () => googleRequests.find((request) => request.method === "POST" && request.url === "/v1/spaces/S/messages"),
    15_000,
    "Google Chat messages.create POST"
  );
...
function buildReport(): Report {
  ...
  if (objectField(inbound, "payload.text") !== "hello from google chat") {
    failedChecks.push("gateway.message.send text mismatch");
  }
  if (objectField(inbound, "payload.conversationKey") !== `google-chat:${threadName}`) {
    failedChecks.push("gateway.message.send conversationKey mismatch");
  }
  if (objectField(inbound, "payload.metadata.channel.platform") !== "google-chat") {
    failedChecks.push("gateway.message.send metadata platform mismatch");
  }
  if (objectField(inbound, "payload.metadata.channel.sourceMessageId") !== messageName) {
    failedChecks.push("gateway.message.send source message mismatch");
  }
  const post = googleRequests.find((request) => request.method === "POST" && request.url === "/v1/spaces/S/messages");
  if (!post) failedChecks.push("mock Google Chat did not receive messages.create POST");
  if (post?.headers.authorization !== "Bearer google-token") {
    failedChecks.push("Google Chat authorization header mismatch");
  }
  const postBody = parseJson(post?.body ?? "");
  if (objectField(postBody, "text") !== "google chat reply") {
    failedChecks.push("Google Chat reply content mismatch");
  }
  if (objectField(postBody, "thread.name") !== threadName) {
    failedChecks.push("Google Chat thread name mismatch");
  }
...
async function startGoogleChatServer(googleRequests: GoogleRequestRecord[]) {
  const server = createServer(async (request, response) => {
    ...
    if (request.method === "POST" && request.url === "/v1/spaces/S/messages") {
      response.writeHead(200, { "content-type": "application/json" });
      response.end(JSON.stringify({ name: "spaces/S/messages/reply" }));
      return;
    }
  ...
  if (!address || typeof address === "string") throw new Error("Google Chat server did not bind a TCP port");

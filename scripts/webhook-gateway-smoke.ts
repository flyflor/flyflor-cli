...
const tempRoot = mkdtempSync(join(tmpdir(), "flyflor-webhook-smoke-"));
const cliHome = resolve(tempRoot, "cli-home");
const gatewayLogPath = resolve(cliHome, "logs", "gateway.log");
const secret = "smoke-secret";
const outputDir = resolve(root, ".flyflor-cli", "webhook-smoke", timestamp());
...
try {
  ...
  gateway = spawn("cargo", ["run", "--quiet", "--", "gateway", "run"], {
    ...
    env: {
      ...
      FLYFLOR_GATEWAY_CHANNELS: "webhook",
      FLYFLOR_LOG: resolve(cliHome, "logs", "channels.log"),
      FLYFLOR_WS_URL: `ws://127.0.0.1:${kernel.port}/ws`,
      WEBHOOK_BIND: `127.0.0.1:${webhookPort}`,
      WEBHOOK_PUBLIC_URL: `http://127.0.0.1:${callback.port}/reply`,
  ...
  const webhookStatus = await postWebhook(webhookPort, {
    id: "smoke-1",
  ...
  await waitFor(() => callbackBodies.length > 0, 15_000, "webhook callback");

  const report = buildReport(webhookStatus);
...
function buildReport(webhookStatus: number): Report {
  const failedChecks: string[] = [];
  if (webhookStatus !== 202) failedChecks.push(`webhook returned ${webhookStatus}`);
  ...
  if (objectField(inbound, "payload.text") !== "hello from webhook") {
    failedChecks.push("gateway.message.send text mismatch");
  }
  if (objectField(inbound, "payload.context.contextForkId") !== "fork-1") {
    failedChecks.push("gateway.message.send context missing");
  }
  if (objectField(inbound, "payload.metadata.webhook.confirmAnswer.choiceId") !== "continue-tools") {
  ...
  if (objectField(callback, "text") !== "webhook reply") failedChecks.push("callback text mismatch");
  if (objectField(callback, "route.platform") !== "webhook") failedChecks.push("callback route mismatch");
...
async function startCallbackServer(callbackBodies: unknown[]) {
  const server = createServer(async (request, response) => {
    ...
    response.end(JSON.stringify({ messageId: "callback-1" }));
  });
...
async function postWebhook(port: number, payload: unknown): Promise<number> {
  const response = await fetch(`http://127.0.0.1:${port}/inbound`, {
    body: JSON.stringify(payload),
    headers: {
      "content-type": "application/json",
      "x-flyflor-webhook-secret": secret,
      "x-flyflor-webhook-source": "smoke",
    },
    method: "POST",
  });
  await response.text();

...
interface Report {
  ...
  matrixRequests: MatrixRequestRecord[];
  ok: boolean;
  outputDir: string;
}

interface MatrixRequestRecord {
  body: string;
...
const tempRoot = mkdtempSync(join(tmpdir(), "flyflor-matrix-smoke-"));
const cliHome = resolve(tempRoot, "cli-home");
const gatewayLogPath = resolve(cliHome, "logs", "gateway.log");
const roomId = "!smoke:example.org";
const outputDir = resolve(root, ".flyflor-cli", "matrix-smoke", timestamp());

mkdirSync(outputDir, { recursive: true });

const kernelMessages: unknown[] = [];
const matrixRequests: MatrixRequestRecord[] = [];
let gateway: ReturnType<typeof spawn> | undefined;
let cleanup: (() => void) | undefined;

try {
  const matrix = await startMatrixServer(matrixRequests);
  ...
  gateway = spawn("cargo", ["run", "--quiet", "--", "gateway", "run"], {
    ...
    env: {
      ...
      FLYFLOR_GATEWAY_CHANNELS: "matrix",
      ...
      MATRIX_ACCESS_TOKEN: "smoke-token",
      MATRIX_ALLOWED_USERS: "@user:example.org",
      MATRIX_HOMESERVER: `http://127.0.0.1:${matrix.port}`,
  ...
  await waitFor(
    () => matrixRequests.find((request) => request.method === "PUT" && request.url.includes("/send/m.room.message/")),
    15_000,
    "matrix send PUT"
  );
...
function buildReport(): Report {
  ...
  if (objectField(inbound, "payload.text") !== "hello from matrix") {
    failedChecks.push("gateway.message.send text mismatch");
  }
  if (objectField(inbound, "payload.conversationKey") !== `matrix:${roomId}`) {
    failedChecks.push("gateway.message.send conversationKey mismatch");
  }
  if (objectField(inbound, "payload.metadata.channel.platform") !== "matrix") {
    failedChecks.push("gateway.message.send metadata platform mismatch");
  }
  if (objectField(inbound, "payload.metadata.channel.sourceMessageId") !== "$event1") {
    failedChecks.push("gateway.message.send source event mismatch");
  }
  const sync = matrixRequests.find((request) => request.method === "GET" && request.url.startsWith("/_matrix/client/v3/sync"));
  if (!sync) failedChecks.push("mock matrix did not receive sync GET");
  const send = matrixRequests.find((request) => request.method === "PUT" && request.url.includes("/send/m.room.message/"));
  if (!send) failedChecks.push("mock matrix did not receive send PUT");
  if (send && objectField(parseJson(send.body), "body") !== "matrix reply") {
    failedChecks.push("matrix send body mismatch");
  }
  if (send && objectField(parseJson(send.body), "msgtype") !== "m.text") {
    failedChecks.push("matrix send msgtype mismatch");
  }
...
async function startMatrixServer(matrixRequests: MatrixRequestRecord[]) {
  let delivered = false;
  const server = createServer(async (request, response) => {
    ...
    if (request.method === "GET" && request.url?.startsWith("/_matrix/client/v3/sync")) {
      ...
      if (!delivered) {
        ...
        response.end(JSON.stringify({ next_batch: "s3", rooms: { join: {} } }));
      }
      return;
    }
    if (request.method === "PUT" && request.url?.includes("/typing/")) {
      response.writeHead(200, { "content-type": "application/json" });
      response.end(JSON.stringify({}));
      return;
    }
    if (request.method === "PUT" && request.url?.includes("/send/m.room.message/")) {
      response.writeHead(200, { "content-type": "application/json" });
      response.end(JSON.stringify({ event_id: "$reply1" }));
      return;
    }
    response.writeHead(404, { "content-type": "application/json" });
    response.end(JSON.stringify({ errcode: "M_NOT_FOUND", error: "not found" }));
  });
  await listen(server, 0);
  const address = server.address();
  if (!address || typeof address === "string") throw new Error("matrix server did not bind a TCP port");

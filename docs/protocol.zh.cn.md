...
当前 `event.subscribe` payload 请求：

- `memory.task_plan.written`

CLI 有意不订阅 `fork.memory.*` 或临时 blackboard event 名称，因为未知 runtime event type 会让 gateway 以 `invalid-payload` 拒绝订阅。Fork memory 仍在 final turn 后或显式命令中通过 `fork.memory.get` 刷新。
...
- `memory.task_plan.written`：标记 plan 数据已更新并请求 `task.list`。

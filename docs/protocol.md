...
Citizen permission choices, including `continue-tools`, `keep-budget`, and `keep-subagents`, are represented as structured `metadata.confirmAnswer` in the outgoing payload. They must not be sent as plain user-message text or as ASK-compatible `metadata.askAnswer`. Normal ASK continuations still use `metadata.askAnswer`.

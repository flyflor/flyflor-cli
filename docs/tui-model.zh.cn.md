...
`continue-tools`、`keep-budget`、`keep-subagents` 等公民权限 options 会展示为 Confirm 授权策略 choices，并作为结构化 metadata 发送。它们不得被转换成普通用户消息文本，也不得使用 ASK 结晶样式展示。普通 ASK continuation 仍然使用 ASK menu 和 continuation metadata。

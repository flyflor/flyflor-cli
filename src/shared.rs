...
pub fn working_light_phase(now_millis: u64) -> usize {
    ((now_millis / 360) % 12) as usize
}

pub fn working_light_bar(width: usize, _phase: usize) -> String {
    "━".repeat(width)
}

pub fn working_light_style(phase: usize, theme: &Theme) -> Style {
    let colors = [theme.pink, theme.purple, theme.blue, theme.purple];
    Style::default().fg(colors[phase % colors.len()])
}

...
const DEFAULT_WEBHOOK_BIND: &str = "127.0.0.1:8790";
...
impl WebhookAdapter {
    pub fn from_env() -> ChannelResult<Self> {
        let secret = env::var("WEBHOOK_SECRET")
            .or_else(|_| env::var("FLYFLOR_WEBHOOK_SECRET"))
            .unwrap_or_default()
            .trim()
            .to_string();
        if secret.is_empty() {
            return Err(ChannelError::missing_config(
                "WEBHOOK_SECRET is required for the webhook channel",
            ));
        }
        let bind = env::var("WEBHOOK_BIND")
            .or_else(|_| env::var("FLYFLOR_WEBHOOK_BIND"))
            .unwrap_or_else(|_| DEFAULT_WEBHOOK_BIND.to_string());
        let callback_url = env::var("WEBHOOK_PUBLIC_URL")
            .or_else(|_| env::var("FLYFLOR_WEBHOOK_PUBLIC_URL"))
            .ok()
            .map(|url| url.trim().to_string())
            .filter(|url| !url.is_empty());
        let allowed_sources = env_set("WEBHOOK_ALLOWED_SOURCES");
        let (tx, rx) = mpsc::channel();
        spawn_webhook_listener(bind, secret.clone(), allowed_sources.clone(), tx)?;
        Ok(Self {
            secret,
            callback_url,
            allowed_sources,
    ...
    fn normalize_payload(
            ...
            && !self.allowed_sources.contains(source)
        {
            return Err(ChannelError::unavailable(format!(
                "webhook source {source} is not allowed"
            )));
        }
        let text = value_string(payload, "text")
            .or_else(|| value_string(payload, "message"))
            .ok_or_else(|| ChannelError::fatal("webhook payload requires text or message"))?;
        let chat_id = value_string(payload, "chatId")
            .or_else(|| value_string(payload, "conversationKey"))
            .or_else(|| value_string(payload, "threadId"))
            .unwrap_or_else(|| "webhook".to_string());
        let user_id = value_string(payload, "userId")
            .or_else(|| {
                payload.get("user").and_then(|user| {
                    value_string(user, "id").or_else(|| value_string(user, "name"))
                })
            })
            .unwrap_or_else(|| source.unwrap_or("webhook-user").to_string());
        let display_name = value_string(payload, "displayName")
            .or_else(|| {
                payload.get("user").and_then(|user| {
                    value_string(user, "displayName").or_else(|| value_string(user, "name"))
                })
            })
            .unwrap_or_else(|| user_id.clone());
        let chat_type = match value_string(payload, "chatType")
            .unwrap_or_default()
            .to_ascii_lowercase()
            .as_str()
        {
            "group" | "channel" => ChatType::Group,
            _ => ChatType::Direct,
        };
        let id = value_string(payload, "id")
            .or_else(|| value_string(payload, "messageId"))
            .unwrap_or_else(|| format!("webhook-{}", now_millis()));
        let source_message_id = id.clone();
        let route = MessageRoute {
            ...
            display_name,
            thread_id: value_string(payload, "threadId").unwrap_or_else(|| chat_id.clone()),
        };
        ...
        let metadata = json!({
            ...
            "webhook": payload.get("metadata").cloned().unwrap_or_else(|| json!({}))
        });
...
impl PlatformAdapter for WebhookAdapter {
    ...
    fn poll_updates(&self) -> ChannelResult<Vec<NormalizedInboundMessage>> {
        let Ok(rx) = self.inbound_rx.lock() else {
            return Err(ChannelError::fatal("webhook inbound queue lock poisoned"));
        };
    ...
    fn send_typing(&self, _route: &MessageRoute) -> ChannelResult<()> {
        Err(ChannelError::unavailable(
            "webhook typing indicator is unavailable",
        ))
    }
...
fn spawn_webhook_listener(
    ...
    let listener = TcpListener::bind(&bind).map_err(|error| {
        ChannelError::unavailable(format!("webhook bind {bind} failed: {error}"))
    })?;
    thread::spawn(move || {
        for stream in listener.incoming() {
            ...
            let tx = tx.clone();
            thread::spawn(move || handle_webhook_stream(&mut stream, &secret, allowed_sources, tx));
        }
    });
    Ok(())
}
    ...
    let response = match read_http_request(stream).and_then(|request| {
        ...
        if header_secret.as_deref() != Some(secret) && bearer_secret.as_deref() != Some(secret) {
            return Err(ChannelError::session_expired(
                "webhook request secret did not match",
            ));
        }
        let payload = serde_json::from_slice::<Value>(&request.body)
            .map_err(|error| ChannelError::fatal(format!("webhook JSON parse failed: {error}")))?;
        let source = request
            .headers
...
fn read_http_request(stream: &mut TcpStream) -> ChannelResult<HttpRequest> {
    ...
    loop {
        ...
        if buffer.len() > MAX_HTTP_BYTES {
            return Err(ChannelError::fatal("webhook request exceeded size limit"));
        }
    ...
    Err(ChannelError::retryable("webhook request ended early"))
}

fn parse_headers(raw: &[u8]) -> ChannelResult<HashMap<String, String>> {
    let text = std::str::from_utf8(raw)
        .map_err(|error| ChannelError::fatal(format!("webhook headers were not UTF-8: {error}")))?;
    let mut lines = text.lines();
    let Some(request_line) = lines.next() else {
        return Err(ChannelError::fatal("webhook request line missing"));
    };
    if !request_line.starts_with("POST ") {
        return Err(ChannelError::fatal("webhook only accepts POST"));
    }
    Ok(lines
...
fn webhook_post(url: &str, secret: &str, payload: Value) -> ChannelResult<Option<Value>> {
    let body =
    ...
    let output = Command::new("curl")
        .args([
            "-sS",
            "--max-time",
            &seconds_arg(WEBHOOK_TIMEOUT_MS),
            "-X",
            "POST",
            url,
            "-H",
            ...
            "-H",
            &format!("X-Flyflor-Webhook-Secret: {secret}"),
            "--data",
            &body,
        ])
        .output()
    ...
    if !output.status.success() {
        ...
        return Err(ChannelError::retryable(format!(
            "webhook callback failed with status {}: {}",
            output.status,
            stderr.trim()
        )));
    }
    ...
    serde_json::from_str::<Value>(text)
        .map(Some)
        .map_err(|error| {
            ChannelError::retryable(format!("webhook callback returned invalid JSON: {error}"))
        })
}
...
mod tests {
    ...
    #[test]
    fn normalizes_webhook_payload_with_context_and_metadata() {
        ...
        let message = adapter
            .normalize_payload(
                &json!({
                    ...
                    "chatId": "room-1",
                    "chatType": "group",
                    "user": { "id": "u-1", "displayName": "User One" },
        ...
        assert_eq!(message.id, "webhook-m-1");
        assert_eq!(message.text, "hello");
        assert_eq!(message.route.platform, "webhook");
        assert_eq!(message.route.chat_type, ChatType::Group);
        assert_eq!(message.route.user_id, "u-1");
        assert_eq!(
            message
                .context
    ...
    #[test]
    fn normalizer_rejects_disallowed_source() {
        let adapter = test_adapter(None, HashSet::from(["allowed".to_string()]));
        let error = adapter
            .normalize_payload(&json!({ "text": "hello" }), Some("blocked"))
            .unwrap_err();
    ...
    fn parses_http_request_and_secret_header() {
        let raw = b"POST / HTTP/1.1\r\nHost: local\r\nX-Flyflor-Webhook-Secret: s\r\nContent-Length: 16\r\n\r\n{\"text\":\"hello\"}";
        let end = find_header_end(raw).unwrap();
        let headers = parse_headers(&raw[..end]).unwrap();

        assert_eq!(
            headers.get("x-flyflor-webhook-secret").map(String::as_str),
            Some("s")
        );
    }

...
mod tests {
    ...
    fn builds_plan_decide_payload() {
        ...
        assert_eq!(payload.get("planId").and_then(Value::as_str), Some("plan-1"));
        assert_eq!(payload.get("action").and_then(Value::as_str), Some("revise"));
        assert_eq!(payload.get("revision").and_then(Value::as_str), Some("补充边界"));
    }
}
